  ...
  try {
    ...
    if (existingRecord.length > 0) {
      return NextResponse.json(
        {
          status: 'error',
          message: 'A record already exists. Only one record is allowed.',
        },
        { status: 409 } // Conflict for existing record
      );
    }
    ...
    const response = await db
      ...
      .returning({
        ...
        name: myschema.PersonalInfo.name,
        subtitle: myschema.PersonalInfo.subtitle,
        bio: myschema.PersonalInfo.bio,
        profile_image: myschema.PersonalInfo.profile_image,
        contact_email: myschema.PersonalInfo.contact_email,
        social_links: myschema.PersonalInfo.social_links,
      });

    // If the response is successful, return success message with inserted data
    return NextResponse.json(
      {
        status: 'success',
        message: 'Data inserted successfully.',
        data: response[0], // Return only the inserted record
      },
